...
static void *_log_do(void *userdata) {
    while (1) {
        int offset = 0;
        char buf[512];
        int amount_read = read(_stdout_pipe[0], buf, sizeof(buf) - 1);

        while (amount_read > 0) {
            buf[offset + amount_read] = 0;
            while (strchr(buf, '\n')) {
                // Print out just the line:
                int lbpos = (strchr(buf, '\n') - buf);
                buf[lbpos] = '\0';
                __android_log_write(ANDROID_LOG_INFO, env_logname, buf);
                // Truncate line from our buffer:
                memmove(buf, buf + lbpos + 1, offset + amount_read + 1 - (lbpos + 1));
                offset = offset + amount_read - (lbpos + 1);
                  // ^ places offset ON last character (not AFTER) since that's '\0'
                if (offset < 0)
                    offset = 0;  // shouldn't happen, but just to be safe
            }
            if (offset >= sizeof(buf) - 1) {
                // Full buffer stuffed with no line break, just echo it:
                buf[sizeof(buf) - 1] = '\0';
                __android_log_write(ANDROID_LOG_INFO, env_logname, buf);
                offset = 0;
            }
...
static int fix_stdout_stderr() {
    ...
    dup2(_stdout_pipe[1], 2);

    // Spawn thread to read pipe:
    if (pthread_create(&log_thread, 0, _log_do, 0) < 0)
        return 0;
    LOGP("Logging thread was launched.");
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    usleep(100000);
    LOGP("Finished python output test.");
    PyRun_SimpleString(
        "import sys\n"
        ...
        "del(_redirect_stdout_stderr)  # keep global scope clean\n"
